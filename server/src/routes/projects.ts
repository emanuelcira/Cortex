...
router.put("/:id", requireAuth, (req, res) => {
  ...
  // Also update collaboration statuses when project completes or drops
...
router.get("/:id/matches", requireAuth, (req, res) => {
  ...
  // Get existing request recipient IDs to exclude
  ...
  // Get existing collaborator IDs to exclude
  ...
  for (const user of allUsers) {
    ...
    // Filter: role must match one of the needed roles
    if (!rolesNeeded.includes(user.role)) continue;

    // Filter: availability must meet project requirement
    ...
    // Score: timezone proximity (max 12h difference)
